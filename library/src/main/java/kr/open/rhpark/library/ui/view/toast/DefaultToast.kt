...
public class DefaultToast(private val context: Context) {
    ...
    public fun msgShort(msg: String): Toast = make(msg, Toast.LENGTH_SHORT)
    public fun msgLong(msg: String): Toast = make(msg, Toast.LENGTH_LONG)
    public fun msgDuration(msg: String,duration:Int): Toast = make(msg, duration)
    ...
    public fun customShowShort(view: View) {
        Toast(context)
    }

    private fun getCustomToast(view:View) = Toast(context).apply {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            this.view = view
        }
    }
